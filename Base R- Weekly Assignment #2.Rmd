...
3. What type of data is this? (e.g. describe what's in the dataset.)
The type of data that billboards have is "songs ranking for Billboard top 100 in the year 2000"

4. Print the column names from this dataset. 
```{r}
data_frame(billboard)
```
...
```
There are 5 song in it

2. In the original dataset, use filter to find information about the song "All the small things." What was the highest position this song reached on the billboard 100? 
```{r}
subset(billboard, billboard$track=="All The Small Things")
Song<- billboard %>% filter(track=="All The Small Things")
```
The highest position this song has reached on the billboard 100 is 6

3. Create a subset of data for songs Tim McGraw sang. Summarize the data for wk 1.
```{r}
subset(billboard, billboard$artist=="Tim McGraw")

Tim1<- billboard %>% filter(wk1=="Tim McGraw")
summary(Tim1)
```
